...
import { DragAndDropProvider, DraggableItem, useAutoScroll } from "@/components/drag-and-drop";
...
export default function SliceSidebar() {
  ...
  return (
    <div class="flex h-full w-64 shrink-0 flex-col overflow-y-auto border-r-2 border-black bg-white">
      ...
      <div class="flex-grow overflow-y-auto" ref={autoScrollRef}>
        {state.slices.length === 0 ? (
          ...
          <DragAndDropProvider
            instanceId="slices-sidebar"
            onDropTargetChange={({ sourceId, targetId }) => {
              if (sourceId === targetId) return;
              const fromIndex = state.slices.findIndex((s) => s.id === sourceId);
              const toIndex = state.slices.findIndex((s) => s.id === targetId);
          ...
          >
            <div class="divide-y">
              <For each={state.slices}>
                {(slice) => (
                  <DraggableItem id={slice.id} data={slice}>
                    {(dragState, ref) => (
                      <div
                        ref={ref}
